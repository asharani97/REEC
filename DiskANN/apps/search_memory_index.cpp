...
#pragma omp parallel for schedule(dynamic, 1)
    ...
    diskann::cout << "Saving query results..." << std::endl;
	for (uint64_t test_id = 0; test_id < query_result_ids.size(); ++test_id) {
		std::string cur_result_path_prefix = result_path_prefix + "_test_" + std::to_string(test_id);
		std::string cur_result_path_ids = cur_result_path_prefix + "_ids.txt";
		std::string cur_result_path_dists = cur_result_path_prefix + "_dists.txt";

		// Save query result IDs to text file
		std::ofstream ids_file(cur_result_path_ids);
		if (ids_file.is_open()) {
			for (uint64_t i = 0; i < query_num; ++i) {
				for (uint64_t j = 0; j < recall_at; ++j) {
					ids_file << query_result_ids[test_id][i * recall_at + j] << " ";
				}
				ids_file << std::endl;
			}
			ids_file.close();
		} else {
			diskann::cerr << "Unable to open file: " << cur_result_path_ids << std::endl;
		}

		// Save query result distances to text file
		std::ofstream dists_file(cur_result_path_dists);
		if (dists_file.is_open()) {
			for (uint64_t i = 0; i < query_num; ++i) {
				for (uint64_t j = 0; j < recall_at; ++j) {
			...
			diskann::cerr << "Unable to open file: " << cur_result_path_dists << std::endl;
		}
	}
	diskann::cout << "Query results saved successfully." << std::endl;
    /* uint64_t test_id = 0;
    for (auto L : Lvec)
    {
        if (L < recall_at)
