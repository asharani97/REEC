...
inline void save_knns_readable(const std::string& filename, int32_t* data,
                                           size_t npts, size_t ndims) {
   

    std::ofstream writer(filename + ".txt");
    if (!writer.is_open()) {
        std::cerr << "Error: Unable to open file: " << filename << ".txt" << std::endl;
        return;
    }
    ...
    for (size_t i = 0; i < npts; ++i) {
        for (size_t j = 0; j < ndims; ++j) {
            writer << std::setw(10) << std::left << data[i * ndims + j];
        }
        writer << std::endl;
    }
    writer << std::endl;
...
inline void save_distance_readable(const std::string& filename,  float* (distances),
                                           size_t npts, size_t ndims) {
    ...
    for (size_t i = 0; i < npts; ++i) {
        for (size_t j = 0; j < ndims; ++j) {
            writer << std::setw(10) << std::left << distances[i * ndims + j];
        }
        writer << std::endl;
    }
             ...
             const diskann::Metric &metric, const std::string &tags_file = std::string(""))
{
    ...
    save_knns_readable("KNNs", closest_points, nqueries, k);
	save_distance_readable("dist_score", dist_closest_points, nqueries, k);
